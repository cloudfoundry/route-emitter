...
var _ = Describe("Watcher", func() {
	...
	BeforeEach(func() {
		...
		bbsClient.SubscribeToEventsR1ByCellIDReturns(eventSource, nil)
	...
	Context("event subscriptions", func() {
		Context("when cell id is set", func() {
			...
			It("subscribes to events for the current cell", func() {
				Eventually(bbsClient.SubscribeToEventsR1ByCellIDCallCount).Should(Equal(1))
				_, cellid := bbsClient.SubscribeToEventsR1ByCellIDArgsForCall(0)
		...
		Context("when the cell id is not set", func() {
			...
			It("subscribes to all events", func() {
				Eventually(bbsClient.SubscribeToEventsR1ByCellIDCallCount).Should(Equal(1))
				_, actualCellID := bbsClient.SubscribeToEventsR1ByCellIDArgsForCall(0)
	...
	Context("handle FlattenedActualLRPRemovedEvent", func() {
		var (
			event models.Event
		)
		...
		BeforeEach(func() {
			...
			event = models.NewFlattenedActualLRPRemovedEvent(actualLRP)
			eventSource.NextReturns(event, nil)
		})
	...
	Context("handle FlattenedActualLRPCreatedEvent", func() {
		var (
			event models.Event
		)
		...
		BeforeEach(func() {
			actualLRP := getActualLRP("process-guid-1", "instance-guid-1", "some-ip", "container-ip", 61000, 5222, false)
			event = models.NewFlattenedActualLRPCreatedEvent(actualLRP)
			eventSource.NextReturns(event, nil)
		})
	...
	Context("handle FlattenedActualLRPChangedEvent", func() {
		var (
			event models.Event
		)
		...
		BeforeEach(func() {
			...
			event = models.NewFlattenedActualLRPChangedEvent(beforeLRP, afterLRP)
			eventSource.NextReturns(event, nil)
		})
	...
	Context("when an unrecognized event is received", func() {
		...
		BeforeEach(func() {
			...
			fakeRawEventSource.NextStub = func() (sse.Event, error) {
			...
			bbsClient.SubscribeToEventsR1ByCellIDReturns(fakeEventSource, nil)
		})
	...
	Context("when eventSource returns error", func() {
		...
		It("resubscribes to SSE from bbs", func() {
			Eventually(bbsClient.SubscribeToEventsR1ByCellIDCallCount, 5*time.Second, 300*time.Millisecond).Should(BeNumerically(">=", 2))
	...
	Context("when subscribe to events fails", func() {
		...
		BeforeEach(func() {
			...
			bbsClient.SubscribeToEventsR1ByCellIDStub = func(logger lager.Logger, cellID string) (events.EventSource, error) {
				select {
		...
		It("retries to subscribe", func() {
			...
			Eventually(bbsClient.SubscribeToEventsR1ByCellIDCallCount, 5*time.Second, 300*time.Millisecond).Should(Equal(2))
	...
	Describe("Sync Events", func() {
		...
		sendEvent := func() {
			Eventually(eventCh).Should(BeSent(EventHolder{models.NewFlattenedActualLRPRemovedEvent(actualLRP1)}))
		}
		...
		Describe("bbs events", func() {
			...
			It("applies cached events after syncing is complete", func() {
				...
				expectedEvent := models.NewFlattenedActualLRPRemovedEvent(actualLRP1)
