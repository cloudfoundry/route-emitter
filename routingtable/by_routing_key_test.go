	...
	Describe("EndpointsByRoutingKeyFromActuals", func() {
		Context("when some actuals don't have port mappings", func() {
			...
			It("should build a map of endpoints, ignoring those without ports", func() {
				...
				Expect(endpoints[endpoint.RoutingKey{ProcessGUID: "abc", ContainerPort: 44}]).To(ConsistOf([]routingtable.Endpoint{
					routingtable.Endpoint{Host: "1.1.1.1", Index: 0, Domain: "domain", Port: 11, ContainerPort: 44},
					routingtable.Endpoint{Host: "2.2.2.2", Index: 1, Domain: "domain", Port: 22, ContainerPort: 44}}))

				Expect(endpoints[endpoint.RoutingKey{ProcessGUID: "abc", ContainerPort: 99}]).To(ConsistOf([]routingtable.Endpoint{
					routingtable.Endpoint{Host: "1.1.1.1", Index: 0, Domain: "domain", Port: 66, ContainerPort: 99},
					routingtable.Endpoint{Host: "2.2.2.2", Index: 1, Domain: "domain", Port: 88, ContainerPort: 99}}))

				Expect(endpoints[endpoint.RoutingKey{ProcessGUID: "def", ContainerPort: 55}]).To(ConsistOf([]routingtable.Endpoint{
					routingtable.Endpoint{Host: "3.3.3.3", Index: 0, Domain: "domain", Port: 33, ContainerPort: 55}}))
			})
		})
		...
		Context("when not all running actuals are desired", func() {
			...
			It("should build a map of endpoints, excluding actuals that aren't desired", func() {
				...
				Expect(endpoints[endpoint.RoutingKey{ProcessGUID: "abc", ContainerPort: 44}]).To(ConsistOf([]routingtable.Endpoint{
					routingtable.Endpoint{Host: "1.1.1.1", Domain: "domain", Port: 11, ContainerPort: 44}}))
				Expect(endpoints[endpoint.RoutingKey{ProcessGUID: "abc", ContainerPort: 99}]).To(ConsistOf([]routingtable.Endpoint{
					routingtable.Endpoint{Host: "1.1.1.1", Domain: "domain", Port: 66, ContainerPort: 99}}))
				Expect(endpoints[endpoint.RoutingKey{ProcessGUID: "def", ContainerPort: 55}]).To(ConsistOf([]routingtable.Endpoint{
					routingtable.Endpoint{Host: "3.3.3.3", Domain: "domain", Port: 33, ContainerPort: 55}}))
			})
		})
	...
	Describe("EndpointsFromActual", func() {
		It("builds a map of container port to endpoint", func() {
			...
			Expect(endpoints).To(ConsistOf([]routingtable.Endpoint{
				routingtable.Endpoint{Host: "1.1.1.1", Domain: "domain", Port: 11, InstanceGuid: "instance-guid", ContainerPort: 44, Evacuating: true, Index: 0},
				routingtable.Endpoint{Host: "1.1.1.1", Domain: "domain", Port: 66, InstanceGuid: "instance-guid", ContainerPort: 99, Evacuating: true, Index: 0},
			}))
		})
	})
